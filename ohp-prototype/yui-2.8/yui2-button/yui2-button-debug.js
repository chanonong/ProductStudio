...
(function () {
    ...
    */
    ...
    var Dom = YAHOO.util.Dom,
        ...
        Lang = YAHOO.lang,
        UA = Lang.merge(YAHOO.env.ua, { ie: document.documentMode ? document.documentMode : YAHOO.env.ua.ie }),
        ...
        */
        _onMouseDown: function (p_oEvent) {
            ...
            if ((p_oEvent.which || p_oEvent.button) == 1) {
                ...
                if (this.hasFocus()) {
					// If environment is not IE remove focus as it interfers with 
					// button getting active state.
					 if (!YAHOO.env.ua.ie) {
						this.blur();
					}
				}
    ...
    YAHOO.extend(YAHOO.widget.ButtonGroup, YAHOO.util.Element, {
        ...
        */
        _onKeyDown: function (p_oEvent) {
			...
			// Bail out if the key is not an arrow key.
			if (nCharCode != KEY.RIGHT && nCharCode != KEY.DOWN && nCharCode != KEY.LEFT && nCharCode != KEY.UP) {
				return;
			}

			// util functions, extracting details
			function getButton() {
				var oTarget = Event.getTarget(p_oEvent),
					sId = oTarget.parentNode.parentNode.id;
				return m_oButtons[sId];
			}

			if (nCharCode == KEY.RIGHT || nCharCode == KEY.DOWN) {
			...
			// If the button is disabled, skip to the next one (but stop once we get back to the currentButton)
			while (nextButton.get('disabled') && nextButton != button) {
				nextButton = getNextButton();
			}

			if (nextButton != button) {
				nextButton.set('checked', true);
				nextButton.focus();
			}     
...
}, '2.8.2' ,{"requires": ["yui2-yahoo", "yui2-dom", "yui2-event", "yui2-element"], "optional": ["yui2-containercore", "yui2-menu"]});
