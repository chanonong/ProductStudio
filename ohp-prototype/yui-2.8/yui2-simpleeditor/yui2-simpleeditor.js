...
var Dom = YAHOO.util.Dom,
    ...
    /**
     * Returns IE>=11 version number or 0 if not IE>=11.
     * @static
     * @return {Object} The version number, or 0 for IE<11 and other browsers.
     */
    function _getModernTridentVersion() {
        var ua = navigator && navigator.userAgent,
            c = 0,
            trident = 0,
            m = ua.match(/Trident\/.*rv:([^\s\)]*)/);

        if (m && m[1]) {
            trident = parseFloat(m[1].replace(/\./g, function() {
                return (c++ == 1) ? '' : '.';
            }));
        }
    ...
    // As IE11 no longer has MSIE signature in the UA string and in order to isolate changes required for simple editor
    // to work in IE>=11 to only this file, declare a new static variable and set it to the IE version number.
    var _modernTrident = _getModernTridentVersion();
    ...
    YAHOO.extend(YAHOO.widget.SimpleEditor, YAHOO.util.Element, {
        ...
        */
        _createIframe: function() {
            ...
            var isrc = 'javascript:;';
            if (this.browser.ie || _modernTrident) {
                //isrc = 'about:blank';
        ...
        */
        _setInitialContent: function(raw) {
            ...
            if (this.browser.ie || _modernTrident || this.browser.webkit || this.browser.opera || (navigator.userAgent.indexOf('Firefox/1.5') != -1)) {
        ...
        _handleKeyDown: function(ev) {
            ...
            switch (ev.keyCode) {
                ...
                case 9:
                    ...
                    //Firefox 3 code
                    if (this.browser.gecko > 1.8 || _modernTrident) {
                        tar = this._getSelectedElement();
                        if (this._isElement(tar, 'li')) {
                            if (ev.shiftKey) {
    ...
    /* {{{  Command Overrides */
        ...
        */
        cmd_inserthtml: function(value) {
            ...
            * inside of the iframe, so we have to place the newly inserted data in the best place that we can.
            */
            if (this.browser.webkit && !this._getDoc().queryCommandEnabled(action) || _modernTrident) {
                this._createCurrentElement('img');
...
}, '2.8.2' ,{"requires": ["yui2-yahoo", "yui2-dom", "yui2-event", "yui2-element"], "optional": ["yui2-containercore", "yui2-dragdrop", "yui2-menu", "yui2-button", "yui2-animation"]});
